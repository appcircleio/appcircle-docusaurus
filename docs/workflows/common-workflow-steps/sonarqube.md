...
slug: /build-integrations/common-integrations/sonarqube
...
|-------------------------------------------------|-------------------------------------------------|
| [**Git Clone**](/build-integrations/common-integrations/git-clone) | If you intend to retrieve the **SonarQube Scanner** parameters from the `sonar-project.properties` file in your repository, it's essential to employ the **Git Clone** step before the **SonarQube** step. After the **Git Clone** step is completed, it generates the `$AC_REPOSITORY_DIR` variable, which is used as input for the **SonarQube** step. |
...
SonarQube accepts `XML` format files to analyse test results. In order to analyse your test results, do not forget to convert the test results to `XML` format by running the [**Convert Xcresult to HTML/XML**](/build-integrations/ios-specific-integrations/convert-xcresult-to-xml-html) step after the [**Xcodebuild for Unit and UI testing**](/build-integrations/ios-specific-integrations/xcodebuild-for-unit-and-ui-test) step.
...
Ensure that the test result path provided to **SonarQube** as a property matches the test result path generated by the [**Android Unit Tests**](/build-integrations/android-specific-integrations/android-unit-tests) step, and direct the output to that location.
