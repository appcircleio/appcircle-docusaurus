...
Although auto-generated `global.yaml` template has "HTTPS enabled" by default, in our sample scenario and configuration it was "HTTPS disabled" to keep it simple to understand. Refer [here](../install-server/docker#3-configure) for sample configuration told at installation.
...
First of all, you need to set `external.scheme` as `https` at `global.yaml` to enable HTTPS for all [subdomains](../install-server/docker#4-dns-settings).
...
Refer to [reset configuration](../install-server/docker#reset-configuration) section for more details.
...
Refer to [installation](../install-server/docker#3-configure) docs for details of `user-secret` usage.
...
Wildcard certificate created for main domain will cover all subdomains listed in [here](../install-server/docker#4-dns-settings).
