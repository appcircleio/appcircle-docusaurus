...
- Go back to Appcircle, enter **Group Attribute Name** as ``http://schemas.auth0.com/org_id`` and **Role Attribute Name** as ``http://schemas.auth0.com/your_namespace_roles``. Note that the role claim is created as a custom claim in Auth0. You must enter the name you determined before.
