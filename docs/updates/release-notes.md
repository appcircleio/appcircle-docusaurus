...
- The Appcircle CLI has undergone a complete revision to make it compatible with the latest Appcircle API. Now it also supports self-hosted Appcircle servers. You can see all the recent changes made in the [changelog](https://github.com/appcircleio/appcircle-cli/blob/main/CHANGELOG.md) and follow [configuration instructions](../self-hosted-appcircle/configure-server/appcircle-cli.md) to use the CLI with a self-hosted Appcircle server. <CloudBadge/> <SelfHostedBadge/>
